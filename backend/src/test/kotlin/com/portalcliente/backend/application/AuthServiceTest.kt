...
private class FakeCredencialRepository : CredencialRepository {
    ...
    override suspend fun buscarPorEmail(email: String): Credencial? = porEmail[email]
    override suspend fun salvarSeNovo(credencial: Credencial): Boolean {
...
class AuthServiceTest {
    ...
    @Test
    fun `cadastro com email novo gera clienteId e permite login`() = runBlocking {
    ...
    @Test
    fun `cadastro com email ja usado rejeita sem sobrescrever credencial existente`() = runBlocking {
        val repository = FakeCredencialRepository()
        val service = novoService(repository)
        val clienteIdOriginal = service.cadastrar("a@b.com", "senha123")

        assertThrows(EmailJaCadastradoException::class.java) {
            runBlocking { service.cadastrar("a@b.com", "outraSenha") }
        }
        assertEquals(clienteIdOriginal, repository.buscarPorEmail("a@b.com")?.clienteId)
    }

    @Test
    fun `login com credenciais validas retorna jwt com sub igual ao clienteId`() = runBlocking {
        val repository = FakeCredencialRepository()
        val service = novoService(repository)
    ...
    @Test
    fun `login com senha incorreta rejeita sem emitir token`() = runBlocking {
        val repository = FakeCredencialRepository()
        val service = novoService(repository)
        service.cadastrar("a@b.com", "senha123")

        assertThrows(CredenciaisInvalidasException::class.java) {
            runBlocking { service.autenticar("a@b.com", "senhaErrada") }
        }
        Unit
    }

    @Test
    fun `login com email nao cadastrado rejeita`() = runBlocking {
        assertThrows(CredenciaisInvalidasException::class.java) {
            runBlocking { novoService().autenticar("ninguem@b.com", "senha123") }
        }
        Unit
    }
}
